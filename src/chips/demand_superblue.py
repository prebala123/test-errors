...
# valid_idx = 44
train_idx = [0, 10, 16, 27, 33, 39, 50, 56, 62, 68]
valid_idx = 40

valid_node_features, valid_edge_features, valid_vn_features, valid_super_vn_features, valid_hypergraph, valid_demand = open_chip(valid_idx)
...
for epoch in range(epochs):
    ...
    total_loss = 0
    for i in train_idx:
        node_features, edge_features, vn_features, super_vn_features, hypergraph, demand = open_chip(i)
    ...
    # with open(filepath, 'a') as f:
    #     f.write(f'{epoch+1},{total_loss/10},{valid_loss.item()/10},{round(time.time()-start_time, 2)}\n')
