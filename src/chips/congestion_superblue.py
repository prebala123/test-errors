...
def open_chip(i):
    ...
    net_features = net_feats['instance_features'][:,:1]
    ...
    congestion = targets['classify'].detach().cpu().numpy()[:,0]
    ...
    return node_features, edge_features, vn_features, super_vn_features, hypergraph, congestion
...
valid_node_features, valid_edge_features, valid_vn_features, valid_super_vn_features, valid_hypergraph, valid_congestion = open_chip(valid_idx)
...
for epoch in range(epochs):
    ...
    for i in train_idx:
        node_features, edge_features, vn_features, super_vn_features, hypergraph, congestion = open_chip(i)

        output = model(node_features, edge_features, vn_features, super_vn_features, hypergraph)
        loss = criterion(output, congestion)
